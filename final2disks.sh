...
echo "Please select two disks for your ZFS mirror setup:"
...
# Create ZFS pool with mirrored data partitions
echo "Creating ZFS pool with mirrored vdevs..."
zpool create -f -o ashift=12 \
 ...
 -O relatime=on \
 -o autotrim=on \
 -m none zroot mirror "$DISK1_PART2" "$DISK2_PART2"
sleep 5
...
efibootmgr --disk "\$DISK1" --part 1 --create --label "ZFSBootMenu-Primary" --loader '\\EFI\\zbm\\zfsbootmenu.EFI' --unicode "spl_hostid=\$(hostid) zbm.timeout=3 zbm.prefer=zroot zbm.import_policy=hostid" --verbose

# Create secondary bootloader entry
efibootmgr --disk "\$DISK2" --part 1 --create --label "ZFSBootMenu-Backup" --loader '\\EFI\\zbm\\zfsbootmenu.EFI' --unicode "spl_hostid=\$(hostid) zbm.timeout=3 zbm.prefer=zroot zbm.import_policy=hostid" --verbose
...
echo "You can now safely reboot into your new ZFS mirrored system."
```
